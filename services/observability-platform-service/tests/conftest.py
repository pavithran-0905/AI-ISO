"""Test fixtures for the observability platform service.
...
**Uses the service's own dedicated database** (``aiios_observability``),
matching the project-wide convention of one Postgres database per service
(see e.g. document-intelligence-service's ``aiios_document_intelligence``)
...
os.environ.setdefault("AIIOS_DATABASE_NAME", "aiios_observability")
...
os.environ.setdefault("AIIOS_REDIS_DB", "37")
...
_TEST_KEY_DIR = Path(tempfile.gettempdir()) / "aiios_observability_platform_test_keys"
...
os.environ.setdefault(
    "AIIOS_OBSERVABILITY_PLATFORM_SERVICE_JWT_PUBLIC_KEY_PATH", str(_TEST_PUBLIC_KEY_PATH)
)
os.environ.setdefault("AIIOS_OBSERVABILITY_PLATFORM_SERVICE_WORKERS_ENABLED", "false")
"""Workers off by default in tests. Every worker is exercised by calling
its ``run_job`` directly, which is both faster and deterministic."""
...
def postgres_test_settings() -> DatabaseSettings:
    return DatabaseSettings(
        ...
        database_port=5433,
        database_name="aiios_observability",
        database_user="aiios",
...
def redis_test_settings() -> RedisSettings:
    """Test db 37 -- this service's own, distinct from every other."""
    ...
    def _headers(
        ...
        *,
        ...
        claims: dict[str, Any] = {
            ...
            "roles": roles if roles is not None else ["observability_admin"],
        }
