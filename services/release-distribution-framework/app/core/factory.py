...
async def _build_workers(
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="release_distribution_framework_scheduler_queue"
    )
    register_build_timeout_sweep(
        manager,
        ReleaseBuildTimeoutSweepWorker(
            session_factory, notifier=notifier, max_age_hours=service.build_max_age_hours
        ).run_job,
        interval_seconds=service.build_timeout_sweep_seconds,
    )
    register_promotion_approval_timeout_sweep(
        manager,
        PromotionApprovalTimeoutSweepWorker(
            session_factory, max_age_hours=service.promotion_approval_max_age_hours
        ).run_job,
        interval_seconds=service.promotion_approval_timeout_sweep_seconds,
    )
    register_lts_support_expiry_sweep(
        manager,
        LtsSupportExpirySweepWorker(
            session_factory, notifier=notifier, warning_days=service.lts_support_expiry_warning_days
        ).run_job,
        interval_seconds=service.lts_support_expiry_sweep_seconds,
    )
    register_eol_schedule_sweep(
        manager,
        EolScheduleSweepWorker(
            session_factory,
            publish=publish_event,  # type: ignore[arg-type]
            notifier=notifier,
            warning_days=service.eol_warning_days,
        ).run_job,
        interval_seconds=service.eol_schedule_sweep_seconds,
    )
    register_statistics_rollup(
        manager,
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    notifier = ReleaseNotifier(notification_manager)
    ...
    logger.info(
        "release-distribution-framework starting up",
        extra={
            "extra_fields": {
    ...
    try:
        ...
        logger.info("release-distribution-framework shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise Release & Distribution Framework",
        description=(
            "Creating, validating, signing, packaging, promoting, publishing, and distributing "
            "all AI-IOS software releases. See this package's own README."
        ),
