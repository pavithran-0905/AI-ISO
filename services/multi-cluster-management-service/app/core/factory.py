...
async def _build_workers(
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="multi_cluster_scheduler_queue"
    )
    register_health_sweep(
        manager,
        HealthSweepWorker(
            ...
            stale_threshold_minutes=service.stale_cluster_threshold_minutes,
        ).run_job,
    ...
    register_policy_reconcile(
        manager,
        PolicyReconcileWorker(
            session_factory,
            notifier=notifier,
            propagation_timeout_seconds=service.policy_propagation_timeout_seconds,
            drift_check_interval_seconds=service.policy_drift_check_interval_seconds,
        ).run_job,
        interval_seconds=service.policy_drift_sweep_seconds,
    )
    register_capacity_analysis(
        manager,
        CapacityAnalysisWorker(
            session_factory,
            notifier=notifier,
            warning_threshold=service.capacity_warning_utilization_fraction,
            critical_threshold=service.capacity_critical_utilization_fraction,
        ).run_job,
        interval_seconds=service.capacity_sweep_seconds,
    )
    register_statistics_rollup(
        manager,
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    notifier = FleetNotifier(notification_manager)
    ...
    logger.info(
        "multi-cluster-management-service starting up",
        extra={
            "extra_fields": {
    ...
    try:
        ...
        logger.info("multi-cluster-management-service shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise Multi-Cluster Management Service",
        description=(
            "Cluster onboarding, provisioning, monitoring, governance, upgrades, and "
            "orchestration across hybrid, edge, and multi-cloud Kubernetes fleets. "
