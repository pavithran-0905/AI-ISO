"""Test fixtures for the multi-cluster management service.
...
**Uses the service's own dedicated database** (``aiios_multi_cluster``),
...
os.environ.setdefault("AIIOS_DATABASE_NAME", "aiios_multi_cluster")
...
os.environ.setdefault("AIIOS_REDIS_DB", "39")
...
_TEST_KEY_DIR = Path(tempfile.gettempdir()) / "aiios_multi_cluster_management_service_test_keys"
...
os.environ.setdefault(
    "AIIOS_MULTI_CLUSTER_MANAGEMENT_SERVICE_JWT_PUBLIC_KEY_PATH", str(_TEST_PUBLIC_KEY_PATH)
)
os.environ.setdefault("AIIOS_MULTI_CLUSTER_MANAGEMENT_SERVICE_WORKERS_ENABLED", "false")
...
def postgres_test_settings() -> DatabaseSettings:
    return DatabaseSettings(
        ...
        database_port=5433,
        database_name="aiios_multi_cluster",
        database_user="aiios",
...
def redis_test_settings() -> RedisSettings:
    """Test db 39 -- this service's own, distinct from every other."""
    ...
    def _headers(
        ...
        *,
        ...
        claims: dict[str, Any] = {
            ...
            "roles": roles if roles is not None else ["cluster_admin"],
        }
