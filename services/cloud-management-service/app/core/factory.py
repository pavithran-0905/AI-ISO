...
async def _build_workers(
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="cloud_management_scheduler_queue"
    )
    register_account_health_sweep(
        manager,
        AccountHealthSweepWorker(
            session_factory, stale_threshold_minutes=service.stale_account_threshold_minutes
        ).run_job,
        interval_seconds=service.account_health_sweep_seconds,
    )
    register_drift_sweep(
        manager,
        DriftSweepWorker(session_factory, publish_event=publish_event, notifier=notifier).run_job,
        interval_seconds=service.drift_sweep_seconds,
    )
    register_budget_sweep(
        manager,
        BudgetSweepWorker(
            ...
            critical_threshold=service.budget_critical_utilization_fraction,
        ).run_job,
        interval_seconds=service.budget_sweep_seconds,
    )
    register_compliance_sweep(
        manager,
        ComplianceSweepWorker(session_factory, notifier=notifier).run_job,
        interval_seconds=service.compliance_sweep_seconds,
    )
    register_statistics_rollup(
        manager,
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    notifier = CloudNotifier(notification_manager)
    ...
    logger.info(
        "cloud-management-service starting up",
        extra={
            "extra_fields": {
    ...
    try:
        ...
        logger.info("cloud-management-service shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise Cloud Management Service",
        description=(
            "Multi-cloud account/resource discovery, provisioning, network/storage/compute/"
            "database/Kubernetes management, IAM, governance, Infrastructure-as-Code tracking, "
            "FinOps, capacity planning, service catalog, and compliance across hybrid and "
            "multi-cloud infrastructure. See this package's own README."
        ),
