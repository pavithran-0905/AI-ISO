...
class CloudStatistic(BaseModel):
    """``cloud_statistics`` -- one rolled-up fleet-wide window.
    ...
    __tablename__ = "cloud_statistics"
    __table_args__ = (
        UniqueConstraint("organization_id", "window_start", name="uq_cloud_statistic_window"),
        Index("ix_cloud_statistic_window", "window_start"),
    )
    ...
    resources_provisioned: Mapped[int] = mapped_column(Integer, default=0)
    total_cost: Mapped[float] = mapped_column(Float, default=0.0)
    budgets_exceeded: Mapped[int] = mapped_column(Integer, default=0)
    drift_detected_count: Mapped[int] = mapped_column(Integer, default=0)
    compliance_violations: Mapped[int] = mapped_column(Integer, default=0)


class CloudReport(BaseModel):
    """``cloud_reports`` -- one generated report."""

    __tablename__ = "cloud_reports"
    __table_args__ = (
        Index("ix_cloud_report_kind", "kind"),
        Index("ix_cloud_report_status", "status"),
    )
...
class CloudAudit(BaseModel):
    """``cloud_audit`` -- the immutable trail.

    Every account registration, resource provisioning, policy change,
    cost change, IaC deployment, compliance change, and administrative
    operation is recorded here, per docs/068's own AUDIT section.
    """

    __tablename__ = "cloud_audit"
    __table_args__ = (
        Index("ix_cloud_audit_time", "occurred_at"),
        Index("ix_cloud_audit_action", "action"),
        Index("ix_cloud_audit_actor", "actor_id"),
    )
...
__all__ = ["CloudAudit", "CloudReport", "CloudStatistic"]
