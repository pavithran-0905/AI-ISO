...
Shared, unmodified, by every job type this service runs (install,
deploy, upgrade, rollback) -- one state machine, not four copies of an
identical one, mirroring
``services/developer-portal-service``'s own shared ``ContentStatus``
precedent (Prompt 074).
"""
...
_S = DeploymentJobStatus

ALLOWED_TRANSITIONS: dict[DeploymentJobStatus, frozenset[DeploymentJobStatus]] = {
...
def validate_transition(
    current: DeploymentJobStatus, target: DeploymentJobStatus
) -> TransitionResult:
    """Whether a deployment job may move from *current* to *target*."""
    current = DeploymentJobStatus(current)
    target = DeploymentJobStatus(target)
...
def is_job_stuck(
    ...
    *,
    ...
    """Whether a job has been ``RUNNING`` past its own configured
    maximum age -- the condition the deployment job timeout sweep
    worker looks for."""
    status = DeploymentJobStatus(status)
