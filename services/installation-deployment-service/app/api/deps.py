"""FastAPI dependency injection for the Installation & Deployment
service.

**Every route requires an administrator role.** Unlike
``services/developer-portal-service`` or
``services/public-api-platform``, there is no "caller acting on their
own account" shape here -- installing, deploying, upgrading, and
rolling back a platform is inherently an operator action, the same
administrator-heavy routing ``services/administration-portal-service``
(Prompt 070) established, not the developer-facing shape 072-074
introduced.
...
from app.config.settings import InstallationDeploymentServiceSettings
...
from app.services.upgrade import UpgradeService
from app.services.verification import VerificationService
from app.types import EventPublisher

_bearer_scheme = HTTPBearer(auto_error=False)

ADMINISTRATOR_ROLES = frozenset({"admin", "administrator", "platform_admin", "installation_admin"})
...
def get_notifier(request: Request) -> DeploymentNotifier:
    return request.app.state.notifier  # type: ignore[no-any-return]


NotifierDep = Annotated[DeploymentNotifier, Depends(get_notifier)]


def get_service_settings(request: Request) -> InstallationDeploymentServiceSettings:
    return request.app.state.service_settings  # type: ignore[no-any-return]


ServiceSettings = Annotated[InstallationDeploymentServiceSettings, Depends(get_service_settings)]
...
def get_installation_session_service(
    repos: Repos, publish: EventPublisherDep, notifier: NotifierDep
) -> InstallationSessionService:
    return InstallationSessionService(
        repos.installation_sessions, publish=publish, notifier=notifier
    )


InstallationSessionServiceDep = Annotated[
    InstallationSessionService, Depends(get_installation_session_service)
]
...
def get_preflight_service(
    ...
    return PreflightService(repos.preflight_results, publish=publish, notifier=notifier)


PreflightServiceDep = Annotated[PreflightService, Depends(get_preflight_service)]


def get_deployment_job_service(
    repos: Repos, publish: EventPublisherDep, notifier: NotifierDep
) -> DeploymentJobService:
    return DeploymentJobService(repos.jobs, repos.history, publish=publish, notifier=notifier)


DeploymentJobServiceDep = Annotated[DeploymentJobService, Depends(get_deployment_job_service)]
...
def get_upgrade_service(
    ...
    job_service: DeploymentJobServiceDep,
) -> UpgradeService:
    return UpgradeService(repos.upgrade_history, job_service, publish=publish, notifier=notifier)
...
def get_rollback_service(
    repos: Repos, publish: EventPublisherDep, job_service: DeploymentJobServiceDep
) -> RollbackService:
    return RollbackService(repos.rollback_history, job_service, publish=publish)
...
def get_verification_service(
    ...
    return VerificationService(repos.verification_results, publish=publish, notifier=notifier)
...
__all__ = [
    ...
    "get_current_user_id",
    "get_db_session",
    "get_deployment_job_service",
    "get_event_publisher",
    "get_installation_session_service",
    "get_notifier",
    "get_organization_id",
