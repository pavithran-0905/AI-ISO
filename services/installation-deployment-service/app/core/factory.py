...
async def _build_workers(
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="installation_deployment_scheduler_queue"
    )
    register_installation_session_expiry_sweep(
        manager,
        InstallationSessionExpirySweepWorker(
            session_factory,
            notifier=notifier,
            max_age_hours=service.installation_session_max_age_hours,
        ).run_job,
        interval_seconds=service.installation_session_expiry_sweep_seconds,
    )
    register_deployment_job_timeout_sweep(
        manager,
        DeploymentJobTimeoutSweepWorker(
            session_factory, notifier=notifier, max_age_hours=service.deployment_job_max_age_hours
        ).run_job,
        interval_seconds=service.deployment_job_timeout_sweep_seconds,
    )
    register_certificate_expiry_sweep(
        manager,
    ...
    register_upgrade_availability_sweep(
        manager,
        UpgradeAvailabilitySweepWorker(
            session_factory,
            notifier=notifier,
            lookback_seconds=service.upgrade_availability_sweep_seconds * 2,
        ).run_job,
        interval_seconds=service.upgrade_availability_sweep_seconds,
    )
    await manager.start()
    return manager
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    notifier = DeploymentNotifier(notification_manager)
    ...
    logger.info(
        "installation-deployment-service starting up",
        extra={
            "extra_fields": {
    ...
    try:
        ...
        logger.info("installation-deployment-service shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise Installation & Deployment Service",
        description=(
            "Infrastructure validation, platform installation, configuration, deployment, "
            "upgrades, rollback, lifecycle management, and Day-0/Day-1 operations. See this "
