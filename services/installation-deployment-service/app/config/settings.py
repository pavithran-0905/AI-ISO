"""Installation & Deployment service settings.
...
class InstallationDeploymentServiceSettings(BaseSettings):
    """Fields specific to this service, not covered by any shared_core section."""

    model_config = SettingsConfigDict(
        env_prefix="AIIOS_INSTALLATION_DEPLOYMENT_SERVICE_", env_file=".env", extra="ignore"
    )

    host: str = Field(default="0.0.0.0")
    port: int = Field(default=8046, ge=1, le=65_535)
    cors_allowed_origins: list[str] = Field(default_factory=list)
    jwt_public_key_path: str = Field(default="keys/jwt_public_key.pem")

    # ---- session/job/certificate thresholds -----------------------------------------------

    installation_session_max_age_hours: int = Field(default=6, ge=1)
    deployment_job_max_age_hours: int = Field(default=4, ge=1)
    certificate_expiry_warning_days: int = Field(default=30, ge=1)

    # ---- workers ------------------------------------------------------------------------------

    workers_enabled: bool = Field(default=True)
    installation_session_expiry_sweep_seconds: int = Field(default=900, ge=60, le=86_400)
    deployment_job_timeout_sweep_seconds: int = Field(default=300, ge=60, le=86_400)
    certificate_expiry_sweep_seconds: int = Field(default=3_600, ge=60, le=604_800)
    statistics_rollup_seconds: int = Field(default=900, ge=60, le=86_400)
    upgrade_availability_sweep_seconds: int = Field(default=3_600, ge=60, le=604_800)
...
class Settings:
    ...
    service: InstallationDeploymentServiceSettings
...
__all__ = ["InstallationDeploymentServiceSettings", "Settings", "build_settings", "get_settings"]
