...
async def _build_workers(
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="public_api_platform_scheduler_queue"
    )
    register_credential_expiry_sweep(
        manager,
        CredentialExpirySweepWorker(
            session_factory,
            notifier=notifier,
            warning_days=service.credential_expiry_warning_days_before,
        ).run_job,
        interval_seconds=service.credential_expiry_sweep_seconds,
    )
    register_quota_reset_sweep(
        manager,
        QuotaResetSweepWorker(
            session_factory,
            notifier=notifier,
            warning_threshold_percent=service.quota_warning_threshold_percent,
        ).run_job,
        interval_seconds=service.quota_reset_sweep_seconds,
    )
    register_api_version_lifecycle_sweep(
        manager,
        ApiVersionLifecycleSweepWorker(session_factory, notifier=notifier).run_job,
        interval_seconds=service.api_version_lifecycle_sweep_seconds,
    )
    register_statistics_rollup(
        manager,
    ...
    register_sandbox_reset_sweep(
        manager,
        SandboxResetSweepWorker(
            session_factory, max_age_hours=service.sandbox_session_max_age_hours
        ).run_job,
        interval_seconds=service.sandbox_reset_sweep_seconds,
    )
    await manager.start()
    return manager
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    notifier = DeveloperNotifier(notification_manager)
    ...
    logger.info(
        "public-api-platform starting up",
        extra={
            "extra_fields": {
    ...
    try:
        ...
        logger.info("public-api-platform shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise Public API & Developer Platform",
        description=(
            "External API exposure, developer onboarding, application/credential management, "
            "API products/plans/subscriptions, rate limiting, quotas, sandbox, SDK publishing "
            "integration, governance, and analytics. See this package's own README."
        ),
