"""Test fixtures for the edge management service.
...
**Uses the service's own dedicated database** (``aiios_edge``), matching
...
os.environ.setdefault("AIIOS_DATABASE_NAME", "aiios_edge")
...
os.environ.setdefault("AIIOS_REDIS_DB", "40")
...
_TEST_KEY_DIR = Path(tempfile.gettempdir()) / "aiios_edge_management_service_test_keys"
...
os.environ.setdefault(
    "AIIOS_EDGE_MANAGEMENT_SERVICE_JWT_PUBLIC_KEY_PATH", str(_TEST_PUBLIC_KEY_PATH)
)
os.environ.setdefault("AIIOS_EDGE_MANAGEMENT_SERVICE_WORKERS_ENABLED", "false")
...
def redis_test_settings() -> RedisSettings:
    """Test db 40 -- this service's own, distinct from every other."""
    ...
    def _headers(
        ...
        *,
        ...
        claims: dict[str, Any] = {
            ...
            "roles": roles if roles is not None else ["device_admin"],
        }
