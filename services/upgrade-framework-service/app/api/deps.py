"""FastAPI dependency injection for the Upgrade Framework service.

**Every route requires an administrator role.** Upgrading, rolling
back, or migrating the platform (or its fleet of managed
infrastructure) is inherently an operator action, the same
administrator-heavy routing
``services/installation-deployment-service`` established for the
adjacent Prompt 075 (and, before that,
``services/administration-portal-service``, Prompt 070).
...
from app.config.settings import UpgradeFrameworkServiceSettings
...
from app.services.upgrade import UpgradeExecutionService, UpgradeJobService
from app.types import EventPublisher

_bearer_scheme = HTTPBearer(auto_error=False)

ADMINISTRATOR_ROLES = frozenset({"admin", "administrator", "platform_admin", "upgrade_admin"})
...
def get_notifier(request: Request) -> UpgradeNotifier:
    return request.app.state.notifier  # type: ignore[no-any-return]


NotifierDep = Annotated[UpgradeNotifier, Depends(get_notifier)]


def get_service_settings(request: Request) -> UpgradeFrameworkServiceSettings:
    return request.app.state.service_settings  # type: ignore[no-any-return]


ServiceSettings = Annotated[UpgradeFrameworkServiceSettings, Depends(get_service_settings)]
...
def get_upgrade_job_service(repos: Repos) -> UpgradeJobService:
    return UpgradeJobService(repos.jobs, repos.history)


UpgradeJobServiceDep = Annotated[UpgradeJobService, Depends(get_upgrade_job_service)]


def get_upgrade_execution_service(
    publish: EventPublisherDep, notifier: NotifierDep, job_service: UpgradeJobServiceDep
) -> UpgradeExecutionService:
    return UpgradeExecutionService(job_service, publish=publish, notifier=notifier)


UpgradeExecutionServiceDep = Annotated[
    UpgradeExecutionService, Depends(get_upgrade_execution_service)
]
...
def get_rollback_service(
    ...
    job_service: UpgradeJobServiceDep,
) -> RollbackService:
    return RollbackService(repos.rollback_history, job_service, publish=publish, notifier=notifier)
...
def get_compatibility_service(
    ...
    return CompatibilityService(repos.compatibility, publish=publish, notifier=notifier)
