...
async def _build_workers(
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="upgrade_framework_scheduler_queue"
    )
    register_upgrade_job_timeout_sweep(
        manager,
        UpgradeJobTimeoutSweepWorker(
            session_factory, notifier=notifier, max_age_hours=service.upgrade_job_max_age_hours
        ).run_job,
        interval_seconds=service.upgrade_job_timeout_sweep_seconds,
    )
    register_migration_timeout_sweep(
        manager,
        MigrationTimeoutSweepWorker(
            session_factory, notifier=notifier, max_age_hours=service.migration_max_age_hours
        ).run_job,
        interval_seconds=service.migration_timeout_sweep_seconds,
    )
    register_release_adoption_sweep(
        manager,
        ReleaseAdoptionSweepWorker(
            session_factory,
            notifier=notifier,
            lookback_seconds=service.release_adoption_sweep_seconds * 2,
        ).run_job,
        interval_seconds=service.release_adoption_sweep_seconds,
    )
    register_health_gate_enforcement(
        manager,
        HealthGateEnforcementWorker(session_factory, notifier=notifier).run_job,
        interval_seconds=service.health_gate_enforcement_sweep_seconds,
    )
    register_statistics_rollup(
        manager,
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    notifier = UpgradeNotifier(notification_manager)
    ...
    logger.info(
        "upgrade-framework-service starting up",
        extra={
            "extra_fields": {
    ...
    try:
        ...
        logger.info("upgrade-framework-service shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise Upgrade Framework Service",
        description=(
            "Orchestrating safe, automated, policy-driven upgrades across the AI-IOS platform, "
            "managed infrastructure, plugins, edge devices, cloud resources, and Kubernetes "
            "environments. See this package's own README."
        ),
