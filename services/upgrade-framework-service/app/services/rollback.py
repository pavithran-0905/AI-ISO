...
Builds on ``UpgradeJobService`` for the underlying job's own generic
lifecycle, adding the rollback-specific
``RollbackStarted``/``RollbackCompleted`` events, a
``rollback_history`` row, and version-path validation via
``app.rollback.engine``. Notifies Rollback Completed directly on
completion.
"""
...
from app.models.upgrade import UpgradeJob
        ...
        *,
        ...
        await self._publish(
            RollbackCompletedEvent(
                ...
                payload={
                    "upgrade_job_id": str(history.upgrade_job_id),
                    "status": status.value,
        ...
        if self._notifier is not None:
            await self._notifier.notify_rollback_completed(to_version=history.to_version)
        return history
