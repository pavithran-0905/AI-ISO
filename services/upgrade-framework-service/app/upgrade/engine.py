...
Shared, unmodified, by every job-shaped table this service tracks
(upgrade jobs, migration history, plugin migrations, rollback
attempts) -- one state machine, not four copies of an identical one,
mirroring ``services/installation-deployment-service``'s own shared
``DeploymentJobStatus`` precedent (Prompt 075).
"""
...
ALLOWED_TRANSITIONS: dict[UpgradeJobStatus, frozenset[UpgradeJobStatus]] = {
...
def validate_transition(current: UpgradeJobStatus, target: UpgradeJobStatus) -> TransitionResult:
    """Whether a job-shaped row may move from *current* to *target*."""
    current = UpgradeJobStatus(current)
    target = UpgradeJobStatus(target)
...
def is_job_stuck(
    status: UpgradeJobStatus, *, started_at: datetime | None, now: datetime, max_age_hours: float
) -> bool:
    ...
    status = UpgradeJobStatus(status)
