...
async def _build_workers(
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="developer_portal_scheduler_queue"
    )
    register_session_expiry_sweep(
        manager,
        DeveloperSessionExpirySweepWorker(session_factory).run_job,
        interval_seconds=service.session_expiry_sweep_seconds,
    )
    register_playground_session_expiry_sweep(
        manager,
        PlaygroundSessionExpirySweepWorker(
            session_factory, max_age_hours=service.playground_session_max_age_hours
        ).run_job,
        interval_seconds=service.playground_session_expiry_sweep_seconds,
    )
    register_search_index_rebuild(
        manager,
        SearchIndexRebuildWorker(session_factory).run_job,
        interval_seconds=service.search_index_rebuild_sweep_seconds,
    )
    register_statistics_rollup(
        manager,
    ...
    register_plugin_submission_staleness_sweep(
        manager,
        PluginSubmissionStalenessSweepWorker(
            session_factory,
            notifier=notifier,
            max_age_hours=service.plugin_validation_max_age_hours,
        ).run_job,
        interval_seconds=service.plugin_submission_staleness_sweep_seconds,
    )
    await manager.start()
    return manager
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    notifier = PortalNotifier(notification_manager)
    ...
    logger.info(
        "developer-portal-service starting up",
        extra={
            "extra_fields": {
    ...
    try:
        ...
        logger.info("developer-portal-service shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise Developer Portal Service",
        description=(
            "Developer onboarding, documentation, interactive API/GraphQL exploration, webhook "
            "testing, SDK distribution, sample projects, code playground, learning center, "
            "plugin publishing, AI documentation assistant, search, and community. See this "
