...
async def _build_workers(
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="administration_portal_scheduler_queue"
    )
    register_health_sweep(
        manager,
        HealthSweepWorker(
    ...
    register_maintenance_sweep(
        manager,
        MaintenanceSweepWorker(session_factory, publish_event=publish_event).run_job,
        interval_seconds=service.maintenance_sweep_seconds,
    )
    register_job_retry_sweep(
        manager,
        JobRetrySweepWorker(
            session_factory, backoff_base_seconds=service.job_retry_backoff_seconds
        ).run_job,
        interval_seconds=service.job_retry_sweep_seconds,
    )
    register_api_key_expiry_sweep(
        manager,
        ApiKeyExpirySweepWorker(session_factory).run_job,
        interval_seconds=service.api_key_expiry_sweep_seconds,
    )
    register_statistics_rollup(
        manager,
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    notifier = AdminNotifier(notification_manager)
    ...
    scheduler_manager = await _build_workers(
        database.session_factory, database.engine, cache.client, app.state.publish_event, settings
    )
    app.state.scheduler_manager = scheduler_manager

    logger.info(
        "administration-portal-service starting up",
        extra={
            "extra_fields": {
    ...
    try:
        ...
        logger.info("administration-portal-service shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise Administration Portal Service",
        description=(
            "The centralized operational control plane for global platform administration, "
            "multi-tenant management, feature flags, platform configuration, diagnostics, "
            "background job management, API management, security administration, "
            "maintenance, and platform health. See this package's own README."
        ),
