...
from app.workers.test_run_timeout_sweep import TestRunTimeoutSweepWorker
...
async def _build_workers(
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="testing_quality_framework_scheduler_queue"
    )
    register_test_run_timeout_sweep(
        manager,
        TestRunTimeoutSweepWorker(
            session_factory, max_age_hours=service.test_run_max_age_hours
        ).run_job,
        interval_seconds=service.test_run_timeout_sweep_seconds,
    )
    register_pipeline_timeout_sweep(
        manager,
        PipelineTimeoutSweepWorker(
            session_factory, notifier=notifier, max_age_hours=service.pipeline_max_age_hours
        ).run_job,
        interval_seconds=service.pipeline_timeout_sweep_seconds,
    )
    register_flaky_test_detection(
        manager,
        FlakyTestDetectionWorker(
            session_factory,
            notifier=notifier,
            lookback_seconds=service.flaky_test_detection_sweep_seconds * 2,
        ).run_job,
        interval_seconds=service.flaky_test_detection_sweep_seconds,
    )
    register_coverage_drop_sweep(
        manager,
        CoverageDropSweepWorker(
            session_factory,
            notifier=notifier,
            drop_threshold_percent=service.coverage_drop_threshold_percent,
            lookback_seconds=service.coverage_drop_sweep_seconds * 2,
        ).run_job,
        interval_seconds=service.coverage_drop_sweep_seconds,
    )
    register_statistics_rollup(
        manager,
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    notifier = QaNotifier(notification_manager)
    ...
    logger.info(
        "testing-quality-framework starting up",
        extra={
            "extra_fields": {
    ...
    try:
        ...
        logger.info("testing-quality-framework shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise Testing & Quality Assurance Framework",
        description=(
            "Validating every AI-IOS component through automated testing, quality gates, "
            "performance validation, security verification, resilience testing, and continuous "
            "quality monitoring. See this package's own README."
        ),
