"""The shared job lifecycle for hardening runs.

One state machine, reused unmodified by ``HardeningRunService`` and
the timeout sweep worker -- the same event-free shared-job-engine shape
``services/performance-benchmark-framework`` (Prompt 078) and every
prior AI-IOS service in this build established.
"""
...
_S = HardeningRunStatus

ALLOWED_TRANSITIONS: dict[HardeningRunStatus, frozenset[HardeningRunStatus]] = {
...
def validate_transition(
    current: HardeningRunStatus, target: HardeningRunStatus
) -> TransitionResult:
    """Whether a hardening run may move from *current* to *target*."""
    current = HardeningRunStatus(current)
    target = HardeningRunStatus(target)
...
def is_job_stuck(
    status: HardeningRunStatus, *, started_at: datetime | None, now: datetime, max_age_hours: float
) -> bool:
    """Whether a hardening run has been ``RUNNING`` past its own
    configured maximum age -- the condition the timeout sweep worker
    looks for."""
    status = HardeningRunStatus(status)
