...
async def _build_workers(
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="license_billing_scheduler_queue"
    )
    register_subscription_renewal_sweep(
        manager,
        SubscriptionRenewalSweepWorker(
            session_factory,
            publish_event=publish_event,
            notifier=notifier,
            grace_period_days=service.subscription_grace_period_days,
            renewal_reminder_days_before=service.renewal_reminder_days_before,
        ).run_job,
        interval_seconds=service.subscription_renewal_sweep_seconds,
    )
    register_license_expiry_sweep(
        manager,
        LicenseExpirySweepWorker(
            session_factory,
            publish_event=publish_event,
            notifier=notifier,
            reminder_days_before=service.license_expiring_reminder_days_before,
        ).run_job,
        interval_seconds=service.license_expiry_sweep_seconds,
    )
    register_quota_reset_sweep(
        manager,
    ...
    register_invoice_generation_sweep(
        manager,
        InvoiceGenerationSweepWorker(
            session_factory, publish_event=publish_event, due_days=service.invoice_due_days
        ).run_job,
        interval_seconds=service.invoice_generation_sweep_seconds,
    )
    register_statistics_rollup(
        manager,
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    notifier = BillingNotifier(notification_manager)
    ...
    logger.info(
        "license-billing-service starting up",
        extra={
            "extra_fields": {
    ...
    try:
        ...
        logger.info("license-billing-service shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise License & Billing Service",
        description=(
            ...
            "licensing models. See this package's own README."
        ),
