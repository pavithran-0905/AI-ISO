"""Test fixtures for the license & billing service.
...
**Uses the service's own dedicated database** (``aiios_billing``), matching
the project-wide convention of one Postgres database per service rather
than the shared bootstrap database referenced in the root ``.env``.
"""
...
os.environ.setdefault("AIIOS_DATABASE_NAME", "aiios_billing")
...
os.environ.setdefault("AIIOS_REDIS_DB", "42")
...
_TEST_KEY_DIR = Path(tempfile.gettempdir()) / "aiios_license_billing_service_test_keys"
...
os.environ.setdefault(
    "AIIOS_LICENSE_BILLING_SERVICE_JWT_PUBLIC_KEY_PATH", str(_TEST_PUBLIC_KEY_PATH)
)
os.environ.setdefault("AIIOS_LICENSE_BILLING_SERVICE_WORKERS_ENABLED", "false")
...
def redis_test_settings() -> RedisSettings:
    """Test db 42 -- this service's own, distinct from every other."""
    ...
    def _headers(
        ...
        *,
        ...
        claims: dict[str, Any] = {
            ...
            "roles": roles if roles is not None else ["billing_admin"],
        }
