"""Test fixtures for the backup & disaster recovery service.
...
**Uses the service's own dedicated database** (``aiios_backup_dr``),
...
os.environ.setdefault("AIIOS_DATABASE_NAME", "aiios_backup_dr")
...
os.environ.setdefault("AIIOS_REDIS_DB", "38")
...
_TEST_KEY_DIR = Path(tempfile.gettempdir()) / "aiios_backup_dr_service_test_keys"
...
os.environ.setdefault("AIIOS_BACKUP_DR_SERVICE_JWT_PUBLIC_KEY_PATH", str(_TEST_PUBLIC_KEY_PATH))
os.environ.setdefault("AIIOS_BACKUP_DR_SERVICE_WORKERS_ENABLED", "false")
...
def redis_test_settings() -> RedisSettings:
    """Test db 38 -- this service's own, distinct from every other."""
    ...
    def _headers(
        ...
        *,
        ...
        claims: dict[str, Any] = {
            ...
            "roles": roles if roles is not None else ["backup_admin"],
        }
