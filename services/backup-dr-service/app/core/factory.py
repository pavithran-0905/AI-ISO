...
async def _build_workers(
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="backup_dr_scheduler_queue"
    )
    register_backup_scheduler(
        manager,
        BackupSchedulerWorker(session_factory, publish_event=publish_event).run_job,
        interval_seconds=service.backup_scheduler_seconds,
    )
    register_retention_sweep(
        manager,
    ...
    register_verification_sweep(
        manager,
        VerificationSweepWorker(
            session_factory,
            max_age_days=service.verification_max_age_days,
            sample_fraction=service.verification_sample_restore_fraction,
        ).run_job,
        interval_seconds=service.verification_sweep_seconds,
    )
    register_replication_monitor(
        manager,
        ReplicationMonitorWorker(
            session_factory,
            warning_threshold_seconds=service.replication_lag_warning_seconds,
            critical_threshold_seconds=service.replication_lag_critical_seconds,
        ).run_job,
        interval_seconds=service.replication_monitor_seconds,
    )
    register_statistics_rollup(
        manager,
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    notifier = BackupDrNotifier(notification_manager)
    ...
    try:
        ...
        logger.info("backup-dr-service shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise Backup & Disaster Recovery Service",
        description=(
            "Backup orchestration, snapshot management, disaster recovery planning, "
            "restore operations, automated failover, replication, compliance "
            "reporting, and recovery validation for every AI-IOS service and "
            "infrastructure component. See this package's own README."
        ),
