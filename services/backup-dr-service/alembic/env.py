...
connects to the same physical ``aiios`` Postgres instance (there is one
shared server, not one per service) but each owns its own database, so
in practice this is defensive rather than load-bearing for the intended
deployment target -- see
``services/observability-platform-service/alembic/env.py``, where the
same defence was found to matter for real after a migration ran against
the shared bootstrap database by mistake and collided with an unrelated
...
VERSION_TABLE = "alembic_version_backup_dr_service"
