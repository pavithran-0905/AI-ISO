"""Test fixtures for the Performance & Benchmark Framework.

Everything runs against **real** PostgreSQL and Redis. Nothing here
mocks infrastructure. RabbitMQ/MinIO are not needed by any test: this
service's event publisher and notifier are both injected as plain
callables/objects (see ``publisher`` below), never resolved from a live
broker in tests.

**Uses the service's own dedicated database**
(``aiios_performance_benchmark_framework``), matching the project-wide
convention of one Postgres database per service rather than the shared
bootstrap database referenced in the root ``.env``.
"""
...
os.environ.setdefault("AIIOS_DATABASE_NAME", "aiios_performance_benchmark_framework")
...
os.environ.setdefault("AIIOS_REDIS_DB", "51")
...
_TEST_KEY_DIR = Path(tempfile.gettempdir()) / "aiios_performance_benchmark_framework_test_keys"
...
os.environ.setdefault(
    "AIIOS_PERFORMANCE_BENCHMARK_FRAMEWORK_JWT_PUBLIC_KEY_PATH", str(_TEST_PUBLIC_KEY_PATH)
)
os.environ.setdefault("AIIOS_PERFORMANCE_BENCHMARK_FRAMEWORK_WORKERS_ENABLED", "false")
...
def postgres_test_settings() -> DatabaseSettings:
    return DatabaseSettings(
        ...
        database_port=5433,
        database_name="aiios_performance_benchmark_framework",
        database_user="aiios",
...
def redis_test_settings() -> RedisSettings:
    """Test db 51 -- this service's own, distinct from every other."""
