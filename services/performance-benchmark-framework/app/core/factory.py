...
async def _build_workers(
    ...
    manager = create_scheduler_framework(
        queue.manager, redis_client, queue_name="performance_benchmark_framework_scheduler_queue"
    )
    register_benchmark_run_timeout_sweep(
        manager,
        BenchmarkRunTimeoutSweepWorker(
            session_factory, max_age_hours=service.benchmark_run_max_age_hours
        ).run_job,
        interval_seconds=service.benchmark_run_timeout_sweep_seconds,
    )
    register_regression_sweep(
        manager,
        RegressionSweepWorker(
            session_factory,
            publish=publish_event,  # type: ignore[arg-type]
            notifier=notifier,
            warning_threshold_percent=service.regression_warning_threshold_percent,
            critical_threshold_percent=service.regression_critical_threshold_percent,
            improvement_threshold_percent=service.improvement_threshold_percent,
            lookback_seconds=service.regression_sweep_seconds * 2,
        ).run_job,
        interval_seconds=service.regression_sweep_seconds,
    )
    register_slo_compliance_sweep(
        manager,
        SloComplianceSweepWorker(
            session_factory,
            publish=publish_event,  # type: ignore[arg-type]
            notifier=notifier,
            lookback_seconds=service.slo_compliance_sweep_seconds * 2,
        ).run_job,
        interval_seconds=service.slo_compliance_sweep_seconds,
    )
    register_capacity_threshold_sweep(
        manager,
        CapacityThresholdSweepWorker(
            session_factory,
            publish=publish_event,  # type: ignore[arg-type]
            notifier=notifier,
            lookback_seconds=service.capacity_threshold_sweep_seconds * 2,
        ).run_job,
        interval_seconds=service.capacity_threshold_sweep_seconds,
    )
    register_statistics_rollup(
        manager,
...
async def _lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    notifier = BenchmarkNotifier(notification_manager)
    ...
    logger.info(
        "performance-benchmark-framework starting up",
        extra={
            "extra_fields": {
    ...
    try:
        ...
        logger.info("performance-benchmark-framework shutting down")
...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        title="AI-IOS Enterprise Performance & Benchmark Framework",
        description=(
            "Benchmarking, profiling, capacity planning, scalability validation, regression "
            "detection, SLO verification, and AI-assisted optimization across the AI-IOS "
            "platform. See this package's own README."
        ),
