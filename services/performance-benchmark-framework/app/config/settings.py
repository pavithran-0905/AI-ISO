"""Performance & Benchmark Framework service settings.
...
class PerformanceBenchmarkFrameworkSettings(BaseSettings):
    """Fields specific to this service, not covered by any shared_core section."""

    model_config = SettingsConfigDict(
        env_prefix="AIIOS_PERFORMANCE_BENCHMARK_FRAMEWORK_", env_file=".env", extra="ignore"
    )

    host: str = Field(default="0.0.0.0")
    port: int = Field(default=8049, ge=1, le=65_535)
    cors_allowed_origins: list[str] = Field(default_factory=list)
    jwt_public_key_path: str = Field(default="keys/jwt_public_key.pem")

    # ---- job thresholds -------------------------------------------------------------------

    benchmark_run_max_age_hours: int = Field(default=4, ge=1)
    regression_warning_threshold_percent: float = Field(default=10.0, ge=0.0)
    regression_critical_threshold_percent: float = Field(default=25.0, ge=0.0)
    improvement_threshold_percent: float = Field(default=10.0, ge=0.0)

    # ---- workers ------------------------------------------------------------------------------

    workers_enabled: bool = Field(default=True)
    benchmark_run_timeout_sweep_seconds: int = Field(default=300, ge=60, le=86_400)
    regression_sweep_seconds: int = Field(default=900, ge=60, le=86_400)
    slo_compliance_sweep_seconds: int = Field(default=900, ge=60, le=86_400)
    capacity_threshold_sweep_seconds: int = Field(default=900, ge=60, le=86_400)
...
class Settings:
    ...
    service: PerformanceBenchmarkFrameworkSettings
...
__all__ = ["PerformanceBenchmarkFrameworkSettings", "Settings", "build_settings", "get_settings"]
