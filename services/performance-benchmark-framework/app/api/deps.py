"""FastAPI dependency injection for the Performance & Benchmark
Framework.
...
administrator-heavy routing ``services/testing-quality-framework``
(Prompt 077), ``services/upgrade-framework-service`` (Prompt 076), and
``services/installation-deployment-service`` (Prompt 075) established
for their own adjacent infrastructure frameworks.
...
from app.config.settings import PerformanceBenchmarkFrameworkSettings
...
ADMINISTRATOR_ROLES = frozenset({"admin", "administrator", "platform_admin", "performance_admin"})
...
def get_notifier(request: Request) -> BenchmarkNotifier:
    return request.app.state.notifier  # type: ignore[no-any-return]


NotifierDep = Annotated[BenchmarkNotifier, Depends(get_notifier)]


def get_service_settings(request: Request) -> PerformanceBenchmarkFrameworkSettings:
    return request.app.state.service_settings  # type: ignore[no-any-return]


ServiceSettings = Annotated[PerformanceBenchmarkFrameworkSettings, Depends(get_service_settings)]
...
def get_benchmark_suite_service(repos: Repos) -> BenchmarkSuiteService:
    return BenchmarkSuiteService(repos.benchmark_suites)


BenchmarkSuiteServiceDep = Annotated[BenchmarkSuiteService, Depends(get_benchmark_suite_service)]


def get_benchmark_run_service(
    repos: Repos, publish: EventPublisherDep, notifier: NotifierDep
) -> BenchmarkRunService:
    return BenchmarkRunService(repos.benchmark_runs, publish=publish, notifier=notifier)


BenchmarkRunServiceDep = Annotated[BenchmarkRunService, Depends(get_benchmark_run_service)]
