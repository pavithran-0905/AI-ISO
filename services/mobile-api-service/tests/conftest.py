"""Test fixtures for the Mobile API service.
...
**Uses the service's own dedicated database** (``aiios_mobile_api``),
...
os.environ.setdefault("AIIOS_DATABASE_NAME", "aiios_mobile_api")
...
os.environ.setdefault("AIIOS_REDIS_DB", "45")
...
_TEST_KEY_DIR = Path(tempfile.gettempdir()) / "aiios_mobile_api_service_test_keys"
...
os.environ.setdefault("AIIOS_MOBILE_API_SERVICE_JWT_PUBLIC_KEY_PATH", str(_TEST_PUBLIC_KEY_PATH))
os.environ.setdefault("AIIOS_MOBILE_API_SERVICE_WORKERS_ENABLED", "false")
...
def postgres_test_settings() -> DatabaseSettings:
    return DatabaseSettings(
        ...
        database_port=5433,
        database_name="aiios_mobile_api",
        database_user="aiios",
...
def redis_test_settings() -> RedisSettings:
    """Test db 45 -- this service's own, distinct from every other."""
...
def minutes_ahead(count: float) -> datetime:
    return utcnow() + timedelta(minutes=count)


def minutes_ago(count: float) -> datetime:
    return utcnow() - timedelta(minutes=count)
