...
/**
 * The asset health table (§6). Columns are exactly the real
 * `AssetResponse` fields this feature has — no "Last Seen" column
 * (confirmed no such field exists on the backend response; `updated_at`
 * stands in and is labeled "Updated," not renamed to imply something
 * it isn't). Sorting maps directly onto `GET /inventory/search`'s own
 * `sort=field:asc|desc` syntax (`docs/frontend/developer-guide/monitoring.md`).
 * Below `md`, falls back to a stacked card list rather than a
 * horizontally-scrolling table (§32).
 */
  ...
  return (
    <div className="flex flex-col gap-3">
            ...
            {assets.map((asset) => (
              <tr key={asset.id} className="border-border hover:bg-muted/50 border-b last:border-0">
                <td className={cellPadding}>
                  <Link
                    href={`/monitoring/assets/${asset.id}`}
      ...
      <ul className="flex flex-col gap-2 md:hidden">
        {assets.map((asset) => (
          <li key={asset.id}>
            <Link href={`/monitoring/assets/${asset.id}`} className="block">
