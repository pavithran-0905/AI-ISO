...
import { ASSET_STATUSES, ASSET_TYPES, type AssetStatusValue, type AssetTypeValue } from "@/features/infrastructure/types";
...
/**
 * Real filters, backed entirely by `GET /inventory/search`'s own
 * server-side `q`/`status`/`asset_type` params. `owner_id`/`project_id`
 * are also real server-side filters, but with no name-resolution
 * endpoint for either (confirmed absent), a raw-UUID text filter
 * wouldn't be a usable control — left out rather than built as
 * decoration. Every input is a real `<label>`-associated control and
 * keyboard-operable.
 */
      ...
      <div className="flex min-w-48 flex-1 flex-col gap-1.5">
        ...
        <div className="relative">
          ...
          <Input
            ...
            placeholder="Name, hostname, IP, MAC, serial, vendor, model, OS…"
            className="pl-9"
          />
        </div>
      </div>
